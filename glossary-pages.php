...
if ( function_exists( 'register_activation_hook' ) ) {
    register_activation_hook( __FILE__, 'glospa_install' );
}
if ( function_exists( 'register_deactivation_hook' ) ) {
    register_deactivation_hook( __FILE__, 'glospa_deactivate' );
}
if ( function_exists( 'register_uninstall_hook' ) ) {
    register_uninstall_hook( __FILE__, 'glospa_uninstall' );
}

function glospa_install() {
    add_option( 'glospa_flush_needed', 1 );
}
function glospa_deactivate() {
    // nothing but do
}
function glospa_uninstall() {
    // nothing but do
}
...
add_action( 'init', function() {
    if ( get_option( 'glospa_flush_needed' ) ) {
        flush_rewrite_rules();
...
add_action('init', function() {
    ...
    add_rewrite_rule(
        '^' . $base_slug . '/([a-zA-Z])/?$',
        'index.php?post_type=glospa-glossary&glossary_letter=$matches[1]',
        'top'
    );
});
...
add_action('wp_enqueue_scripts', function() {
    wp_enqueue_style( 'glossary-pages-css', plugins_url( 'assets/css/glossary.css', __FILE__ ) );
});
...
add_action('template_redirect', function() {
    ...
    if ($letter && get_query_var('post_type') === 'glospa-glossary') {
        // Force the list
        status_header(200);
        nocache_headers();
        // Call a specific function to render the page
        glospa_render_letter_page(strtoupper($letter));
        exit;
    }
});
...
function glospa_render_letter_page($letter) {
    get_header();
    echo '<div class="glossary-page wrap">';
...
// Display the custom field on the single page "glospa-glossary"
add_filter('the_content', function($content) {
    if ( is_singular('glospa-glossary') && in_the_loop() && is_main_query() ) {
        $synonym = get_post_meta(get_the_ID(), '_glossary_pages_synonym', true);
        $example = get_post_meta(get_the_ID(), '_glossary_pages_example', true);

        $output = '';
        if ($synonym || $example) {
            $output .= '<div class="glossary-single-fields">';
            if ($synonym) {
...
add_action('admin_menu', function() {
    add_submenu_page(
        'edit.php?post_type=glospa-glossary',            // parent slug
        esc_html__('Glossary Pages Help', 'glossary-pages'),   // page title
        esc_html__('Help', 'glossary-pages'),                  // menu title
        'manage_options',                                      // capability
        'glospa-glossary-pages-help',                          // menu slug
        'glospa_render_help_page'                              // callback
    );
});


// Content of the help page
function glospa_render_help_page() {
    ?>
    <div class="wrap">
        ...
        <dl>
            <dt><code>[glossary_pages_nav]</code></dt>
            <dd>
            ...
            <dt><code>[glossary_pages_list]</code></dt>
            <dd>
                <?php esc_html_e('Displays the full list of glossary terms, grouped by starting letter.', 'glossary-pages'); ?>
                <br>
                <strong><?php esc_html_e('Available attributes:', 'glossary-pages'); ?></strong>
                <ul style="margin-top:0.2em;">
                    <li><code>category="slug"</code> — <?php esc_html_e('Filter glossary terms by category slug (optional). Example: [glossary_pages_list category="seo"]', 'glossary-pages'); ?></li>
                </ul>
            </dd>

            <dt><code>[glossary_pages_categories]</code></dt>
            <dd>
            ...
            <dt><code>[glossary_pages_search]</code></dt>
        ...
        <pre style="background: #f7f7f7; padding: 1em; border-radius: 8px;">[glossary_pages_nav]
[glossary_pages_list]</pre>
